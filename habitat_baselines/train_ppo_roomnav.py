...
from habitat.datasets.pointnav.roomnav_dataset import RoomNavDatasetV1
...
class NavRLEnv(habitat.RLEnv):
    ...
    def get_info(self, observations):
        ...
        if self.get_done(observations):
            info["roomnavmetric"] = self.habitat_env.get_metrics()["roomnavmetric"]

        return info


def make_env_fn(config_env, config_baseline, rank):
    dataset = RoomNavDatasetV1(config_env.DATASET)
    config_env.defrost()
...
def construct_envs(args):
    ...
    scenes = RoomNavDatasetV1.get_scenes_to_load(basic_config.DATASET)
    ...
    for i in range(args.num_processes):
        ...
        if len(scenes) > 0:
            config_env.DATASET.ROOMNAVV1.CONTENT_SCENES = scenes[
